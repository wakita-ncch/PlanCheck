...
//         :  2017/05/08 by wakita
//            Modification for ESAPI13.7 from 11.0
//         :  2017/12/04 bt wakita
//            Add checking if isocenters are the same in plan sum.
...
namespace VMS.TPS
{
  ...
  class Script
  {
	...
	throw new ApplicationException(error+"This plan has not been calculated!!!"+endline);
      }
      ...
      {
	if(IsPlanSum) planSumWindow.Close();
	throw new ApplicationException(error+"The selected plan does not reference a StructureSet."+endline);
      }
      ...
      {
	results.Add(new Pair(warning+"Orientation is not Head First Supine!"+endline, "ERROR"));
	fAllOK = false;
      }
      ...
      string number = null;
      if(!match1.Success)
	results.Add(new Pair(error+"Course ID must be \"Course#\""+endline, "ERROR"));
      ...
      if(!match2.Success)
	results.Add(new Pair(error+"Plan ID must be \"#-#-#\""+endline, "ERROR"));
      else if(number != match2.Groups["number"].Value)
	results.Add(new Pair(error+"First number of Plan ID\nmust be same as Course number"+endline, "ERROR"));
	...
	results.Add(new Pair(string.Format("Plan ID is...\n{0}", planID), "NORMAL"));
      ...
      {
	if(IsPlanSum) planSumWindow.Close();
	throw new ApplicationException(error+"Input Number of Fractions!!!"+endline);
      }
      if (Double.IsNaN(frdose1))
      {
	if(IsPlanSum) planSumWindow.Close();
	throw new ApplicationException(error+"Input Dose/Fraction!!!"+endline);
      }

      if (NoF1>49)
      {
	results.Add(new Pair(warning+"Number of fraction is too many!!"+endline, "ERROR"));
	fAllOK = false;
      }

      if (frdose1<100)
      {
	results.Add(new Pair(warning+"Dose per fraction is too small!!"+endline, "ERROR"));
	fAllOK = false;
      }

      // Check CT origin
      if(!((image.UserOrigin.x==0) && (image.UserOrigin.y==0) && (image.UserOrigin.z==0))){
	results.Add(new Pair(string.Format(error+"CT origin is ({0}, {1}, {2})"+endline, image.UserOrigin.x/10, image.UserOrigin.y/10, image.UserOrigin.z/10), "ERROR"));
	fAllOK = false;
      }
	...
	results.Add(new Pair(warning+"ID of CT image dose not contain the\ncorrect date in a format of \"YYMMDD\"."+endline, "ERROR"));
	fAllOK = false;
      }else{
	...
	if(datetime > creation && ss.Image.Series.ImagingDeviceId != "" && ss.Image.Series.Study.Comment != "ARIA RadOnc Study")
	{
      ...
      if(!fIsCTLatest)
	results.Add(new Pair(warning+"Planning CT image is not latest!!"+endline, "ERROR"));
	...
	if(!beam.IsSetupField) numberOfBeams += 1;
      ...
      {
	if(IsPlanSum) planSumWindow.Close();
	throw new ApplicationException(error+"Only 1 field is allowed in electron plan!!!"+endline);
      }
      if(fIsElectron && weight != 1.0)
	results.Add(new Pair(warning+"Field weight is not 1.0\nin electron plan!!!"+endline, "ERROR"));
      if(fIsElectron && n_method.IndexOf("No plan") < 0)
	results.Add(new Pair(warning+"No Plan Normalization is recommended\nin electron plan!!!"+endline, "ERROR"));


      // Check Machine ID
      String machine = beams.ElementAt(0).TreatmentUnit.Id;
	...
	if(fIsVMAT){
	  ...
	  if(!fWithCouch){
	    results.Add(new Pair(string.Format(warning+"No Couch Structures!!!"+endline), "ERROR"));
	    fAllOK = false;
	  }else{
	...
	if(!beam.IsSetupField)
	{
	  ...
	  if(beam.MLC != null && beam.MLCPlanType == 0){
	    ...
	    if(Math.Abs(jaws.X1-MinX) > 2.0){
	      results.Add(new Pair(string.Format(warning+"Check X1 Jaw/MLC!!!"+endline), "ERROR"));
	      fField = false;
	    }
	    if(Math.Abs(jaws.X2-MaxX) > 2.0){
	      results.Add(new Pair(string.Format(warning+"Check X2 Jaw/MLC!!!"+endline), "ERROR"));
	      fField = false;
	    }
	    if((jaws.Y1-MinY_L < -2.0) || (jaws.Y1 > MinY_U)){
	      results.Add(new Pair(string.Format(warning+"Check Y1 Jaw/MLC!!!"+endline), "ERROR"));
	      fField = false;
	    }
	    if((MaxY_U-jaws.Y2 < -2.0) || (jaws.Y2 < MaxY_L)){
	      results.Add(new Pair(string.Format(warning+"Check Y2 Jaw/MLC!!!"+endline), "ERROR"));
	      fField = false;
	    }
	    ...
	    if(beam.EnergyModeDisplayName == "4X"){
	      if(doseRate != 250) {
		results.Add(new Pair(string.Format(warning+"Dose Rate is {0} MU/min!!"+endline, doseRate), "ERROR"));
		fField = false;
	      }
	    ...
	    if(beam.WeightFactor == 0.0) {
	      results.Add(new Pair(string.Format(warning+"{0} has 0 MU!!"+endline, beam.Id), "ERROR"));
	      fField = false;
	    }
	...
	if(VVector.Distance(beam.IsocenterPosition,isocenter)!=0) fIsICSame = false;
      ...
      if(!fIsICSame){
	results.Add(new Pair(warning+"This plan has two or more isocenters!!"+endline, "ERROR"));
	fAllOK = false;
      }
      // Check the CT origin is inside the BODY
      foreach(var structure in structureSet.Structures){
	if((structure.DicomType == "EXTERNAL") && !structure.IsPointInsideSegment(image.UserOrigin)){
	  results.Add(new Pair(warning+"CT origin is outside the BODY!!"+endline, "ERROR"));
	  fAllOK = false;
	}
      }
      ...
      if(distance > 500){
	results.Add(new Pair(warning+"The distance from IC to CT origin is too far."+endline, "ERROR"));
	fAllOK = false;
      }
	...
	if(refpoint.IsPrimaryReferencePoint){
      ...
      {
	if(IsPlanSum) planSumWindow.Close();
	throw new ApplicationException(warning+"This plan has no reference points!!"+endline+"\n\n");
      }
      if (!fHasPrimaryRP) 
      {
	if(IsPlanSum) planSumWindow.Close();
	throw new ApplicationException(warning+"This plan has no Primary Reference Point!!"+endline+"\n\n");
      }
      if (VVector.Distance(refPointLocation, planSetup.PlanNormalizationPoint) > 0.01 && !fIsVMAT && !fIsIMRT) 
      {
	if(IsPlanSum) planSumWindow.Close();
	throw new ApplicationException(warning+"Primary Reference Point is not\nNormalization point!!"+endline+"\n\n");
      }
      if (Math.Abs(fr1.DosePerFractionInPrimaryRefPoint.Dose-frdose1)>1e-5 && !fIsVMAT && !fIsIMRT && !fIsSBRT && !fIsElectron)
      {
	results.Add(new Pair(warning+"Check Plan Normalization Mode!!"+endline, "ERROR"));
	fAllOK = false;
      }
      ...
      {
	if(IsPlanSum) planSumWindow.Close();
	throw new ApplicationException(warning+"Primary Reference Point has no location!!"+endline+"\n\n");
      }
//	fAllOK = false;
      }else if (VVector.Distance(isocenter, refPointLocation) > 0.1 && !fIsElectron && !fIsVMAT && !fIsIMRT){
	results.Add(new Pair(warning+"Isocenter and Primary reference point\nare not the same!!"+endline, "ERROR"));
	fAllOK = false;
	...
	results.Add(new Pair(warning+"IMRT/SBRT/Electron plan does not need\nto have reference point location."+endline, "ERROR"));
      }
	...
	foreach(var beam in beams)
	{
	  if(!beam.IsSetupField)
	  {
	    ...
	    if(result == 1) results.Add(new Pair(string.Format(warning+"Reference point is too close to\nfield edge of {0}!"+endline,beam.Id), "ERROR"));
	    if(result == 2) results.Add(new Pair(string.Format(warning+"Reference point is outside of\nfield edge of {0}!"+endline,beam.Id), "ERROR"));
	  }
	}
      }
	...
	if(profile[1] < -300) results.Add(new Pair(warning+"HU value at Reference point is too low!"+endline, "ERROR"));
      }
	...
	if(structure.DicomType == "BOLUS"){
	  ...
	  if (!fLinkBolus) {
	    results.Add(new Pair(warning+"Some Fields are not linked to Bolus!!"+endline, "ERROR"));
	    fAllOK = false;
	  }else {
	...
	if(!fHasBolus || (fHasBolus && !fLinkBolus))
	{
	  ...
	  {
	    results.Add(new Pair(warning+"SSD is not 100.0 cm!!"+endline, "ERROR"));
	    fAllOK = false;
	  }
	}else{
	  if(ssd > 1005.5 || ssd <= 1004.5)
	  {
	    results.Add(new Pair(warning+"SSD (Bolus+) is not 100.5 cm!!"+endline, "ERROR"));
	    fAllOK = false;
	  }
	}
      }
...
//      window.Height = 900;
    ...
    private bool CheckIsocenter(PlanSum planSum)
    {
      bool IsSingleIC = true;
      var isocenter = planSum.PlanSetups.ElementAt(0).Beams.ElementAt(0).IsocenterPosition;
      foreach(PlanSetup planSetup in planSum.PlanSetups)
      {
	if(VVector.Distance(planSetup.Beams.ElementAt(0).IsocenterPosition, isocenter) >= 0.01)
	  IsSingleIC = false;
      }

      if(!IsSingleIC)
      {
	Window isoWindow = new Window();
	var panel = new StackPanel();

	var check = new Label();
	check.Content = "Isocenter is not the same!!";
	check.Margin = new Thickness(10,10,10,10);
	check.FontSize = 18;
	...
	isoWindow.Content = panel;
	isoWindow.Title = "Isocenter Check";
	isoWindow.SizeToContent = SizeToContent.WidthAndHeight;
	isoWindow.MaxWidth = 300;
	isoWindow.MaxHeight = 200;
	isoWindow.WindowStartupLocation = WindowStartupLocation.CenterScreen;
	isoWindow.ShowDialog();
    ...
    CheckBox ic_check = new CheckBox();
    ...
    private void InitializeUI(Window window, Patient patient)
    {
      ...
      planSumCombo.Margin = new Thickness(10, 0, 10, 10);
      planSumCombo.FontSize = 15;
      panel.Children.Add(planSumCombo);

      ic_check.Content = "Single Isocenter";
      ic_check.Margin = new Thickness(10, 0, 10, 15);
      ic_check.FontSize = 15;
      panel.Children.Add(ic_check);
...
//      subpanel.Children.Add(nulllabel);
	...
	return;
      ...
      {
	if(ic_check.IsChecked == true)
	{
	  if(CheckIsocenter(selectedPlanSum))
	  {
